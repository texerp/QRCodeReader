...
class CameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, ItemSelectionViewControllerDelegate {
	...
	override func viewDidLoad() {
		super.viewDidLoad()
         ...
         Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(self.scanQRCodeRequest), userInfo: nil, repeats: true)
    ...
    func checkServiceCondition(qrcode: String)  {
        if !checkRequestDuplicatePermin(qrcode: qrcode) {
            CallAPIQRCodeTick(qrcode: qrcode)
        }
    }
    func CallAPIQRCodeTick ( qrcode: String ) {
    ...
    func checkRequestDuplicatePermin( qrcode: String ) -> Bool{
        let date = Date()
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy/MM/dd HH:mm"
        let dateString = formatter.string(from: date)
        
        let requestHashModel = RequestHashable(_code: qrcode, _dateTime: dateString)
        print(requestHashModel.hashValue)
        if arrHash.count > 20 {
            arrHash = Array(arrHash.dropFirst(10))
        }
        
        if arrHash.contains(requestHashModel.hashValue) {
            return true
        }else {
            arrHash.append(requestHashModel.hashValue)
            return false
        }
    }
	/**
	...
	private func createMetadataObjectOverlayWithMetadataObject(_ metadataObject: AVMetadataObject) -> MetadataObjectLayer {
		...
		if let barcodeMetadataObject = transformedMetadataObject as? AVMetadataMachineReadableCodeObject {
            ...
            // Callllllllll
                ...
                checkServiceCondition(qrcode: stringValue)
